...
function saveStates(w,data,file,epoch;pdrop=0.5)
    ...
    for (batch,(x,y)) in enumerate(data)
        x = predict(w,x;pdrop=pdrop)
        write(file,"batch $batch epoch $epoch\n")

        for j=1:length(y)
            write(file,"$(convert(Int64,y[j])) $(findmax(x[:,j])[2])\n")            
        end
...
function main(args="")
    ...
    @add_arg_table s begin
        ...
        ("--output"; arg_type=String; default="gp_"*Dates.format(now(),"yyyy-mm-dd-HH-MM-SS")*".txt"; help="saves the prediction.")
...
PROGRAM_FILE == "get_predictions.jl" && main(ARGS)
