...
function main(args="")
    ...
    @add_arg_table s begin
        ...
        ("--trials"; arg_type=Int; default=5 ; help="number of trials.")
        ("--output"; arg_type=String; default=Dates.format(now(),"yyyy-mm-dd-HH-MM-SS")*".txt"; help="saves the performance measures.")
    ...
    performanceV=Performance[Performance(0,0.0,0.0) for i in 1:stride]
    
    
    w = weights(o[:hidden]...; atype=atype, winit=o[:winit])
    @time for epoch=0:stride*o[:trials]-1
        ...
        performanceV[epoch%stride+1].cnt+=1
        performanceV[epoch%stride+1].trn+=accuracy(w,dtrn,predict)
        performanceV[epoch%stride+1].tst+=accuracy(w,dtst,predict)

        println(epoch," ",epoch%stride+1," ",accuracy(w,dtrn,predict)," ",accuracy(w,dtst,predict))
        
    end


    outputFile=open(o[:output],"w")
    for (i,p) in enumerate(performanceV)
        write(outputFile,"$(i-1) $(p.trn/p.cnt) $(p.tst/p.cnt)\n")
        println("$(i-1) $(p.trn/p.cnt) $(p.tst/p.cnt)")
    end
    close(outputFile)
...
PROGRAM_FILE == "fashion-mnist_graph.jl" && main(ARGS)
